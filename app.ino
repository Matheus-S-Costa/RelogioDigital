...
void loop() {
  ...
  int botaoDefinir = digitalRead(pinoDefinirAlarme);

  if (botaoDefinir == HIGH) {
    definirProximoAlarme = true;
    delay(300);
  }

  if (definirProximoAlarme) {
    atualizarHoraDespertador();
  }

  lidarComPressaoBotao(botaoAcHora, botaoDecHora, botaoAcMin, botaoDecMin);
...
void lidarComPressaoBotao(int acHora, int decHora, int acMin, int decMin) {
  if (acHora == HIGH) {
    incrementarHora();
  }
